...
//        add_filter( 'fl_builder_render_css',                	__CLASS__ . '::postGridCSS', 10, 2 );
    ...
    public static function loadLayoutPath($path, $layout, $settings)
    {
        if ('theme' === $layout) {
            ...
            $temp_path = FLBuilder::MODULE_DIR . "post-grid/includes/post-theme-{$post_type}.php";
            $path      = file_exists($temp_path) ? $temp_path : FLBuilder::MODULE_DIR . 'post-grid/includes/post-theme.php';
        }
        return $path;
    }
    ...
    public static function filterBar($settings)
    {
        ...
        $temp_path  = FLBuilder::MODULE_DIR . "post-grid/includes/filter-bar-{$post_type}.php";
        $path       = file_exists($temp_path) ? $temp_path : FLBuilder::MODULE_DIR . 'post-grid/includes/filter-bar.php';
